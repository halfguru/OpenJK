...
void misc_model_cargo_die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod, int dFlags, int hitLoc )
{
	...
	if (( flags & DROP_MEDPACK ))
	{
		...
		if ( health )
		{
			...
			LaunchItem( health, temp, (float *)vec3_origin, nullptr );
		}
	}
	if (( flags & DROP_SHIELDS ))
	{
		...
		if ( shields )
		{
			...
			LaunchItem( shields, temp, (float *)vec3_origin, nullptr );
		}
	}
	...
	if (( flags & DROP_BACTA ))
	{
		...
		if ( bacta )
		{
			...
			LaunchItem( bacta, temp, (float *)vec3_origin, nullptr );
		}
	}
	...
	if (( flags & DROP_BATTERIES ))
	{
		...
		if ( batteries )
		{
			...
			LaunchItem( batteries, temp, (float *)vec3_origin, nullptr );
		}
	}
}
